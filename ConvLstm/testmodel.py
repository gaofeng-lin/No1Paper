...
# 训练集从（82-91）-（92-101）为一组，直到（321-330）-（331-340）结束
# 验证集从（322-331）-（332-341）为一组，直到（401-410）-（411-420）结束
# 测试集从（402-411）-（412-321）为一组，直到（481-490）-（491-500）结束
# 总共取400个数据，240个训练、验证和测试各80
...
class ScalarDataSet():
    ...
    def ReadDataV2(self):
        ...
        for j in range(0, self.end):
            low = np.zeros((1,self.dimLow[0],self.dimLow[1]))
            for i in range(self.start+j, self.start+20+j):
...
def train():
    ...
    with torch.no_grad():    
        for i, (inputs, labels) in enumerate(testLoader):
            ...
            with open(f'/home/nvme0/lgf/TianCi/ConvLSTM-PyTorch/save_diff_file/2023-07-24-10-03-08/{i}.dat', 'w') as f:
                f.write('variables="x","y","u"\n')
                f.write('    ZONE T="Small Rect", I= 480 J= 120\n')
                for y in range(0, 120):
                    for x in range(0, 480):
